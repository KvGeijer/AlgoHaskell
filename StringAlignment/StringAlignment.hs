...
doQuery costs charMap (string1, string2) = result where
    ...
    result = let (dif1,dif2) = difres in (D.toList dif1, D.toList dif2)
...
complStrings :: Array Int (Array Int Int) -> Map.Map Char Int -> Array (Int,Int) Int -> Array Int Char -> Array Int Char -> Int -> Int ->  (D.DList Char, D.DList Char)
complStrings costs charMap optArr string1 string2 i j
    | i == 0 && j == 0                                    = (D.empty, D.empty)
    | j == 0                                              = let (s1, s2) = curried (i-1) j in (D.snoc s1 char1, D.snoc s2 '*')
    | i == 0                                              = let (s1, s2) = curried i (j-1) in (D.snoc s1 '*', D.snoc s2 char2)
    | getCost costs charMap (string1 ! i) (string2 ! j) + optArr ! (i-1, j-1) == optArr ! (i,j)     = let (s1, s2) = curried (i-1) (j-1) in (D.snoc s1 char1, D.snoc s2 char2)          -- This line is not nice. But not necceceary either. Just to get the tests right, is the same as "otherwise" but the tests wanted it this way.
    | optArr ! (i,j) == (skipCost + optArr ! (i, j-1))    = let (s1, s2) = curried i (j-1) in (D.snoc s1 '*', D.snoc s2 char2)
    | optArr ! (i,j) == (skipCost + optArr ! (i-1, j))    = let (s1, s2) = curried (i-1) j in (D.snoc s1 char1, D.snoc s2 '*')
    | otherwise                                           = let (s1, s2) = curried (i-1) (j-1) in (D.snoc s1 char1, D.snoc s2 char2)
...
test1 = do
    ...
    doQueries costs charMap [("AABC","ABC")]
